...
export function getMembershipNftAddressOrNull(): string | null {
  const raw =
    process.env.NEXT_PUBLIC_MEMBERSHIP_NFT_ADDRESS?.trim() ||
    process.env.NEXT_PUBLIC_MEMBERSHIP_CONTRACT_ADDRESS?.trim();
  if (!raw) return null;
  try {
...
export function getMembershipNftAddress(): string {
  ...
  if (!addr) {
    throw new Error(
      'Set NEXT_PUBLIC_MEMBERSHIP_NFT_ADDRESS in .env.local to your localhost deployed contract address.',
    );
  }
  return addr;
}
